		...
		for (int i = 0; i < nodes.length; i++) {
			if (nodes[i].getId() == 0) {
				...
				try {
					...
					throw new IOException("Could not add triple s=" + nodes[0].getNodeString() + ",p=" + nodes[1].getNodeString() + ",o=" + nodes[2].getNodeString() + " because of an Exception on adding Node at position " + i, e);
				}
			}
